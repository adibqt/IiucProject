...
const Jobs = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      navigate('/login', { state: { from: '/jobs', message: 'Please login to view jobs' } });
      return;
    }
    loadData();
  }, [navigate]);
  ...
  const loadData = async () => {
    try {
      ...
      try {
        const skillsResponse = await axios.get(`${API_BASE_URL}/skills`, config);
      ...
      if (error.response?.status === 401) {
        ...
        navigate('/login', { state: { from: '/jobs', message: 'Your session has expired. Please login again.' } });
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleJobClick = async (job) => {
    ...
    try {
      ...
      // Update local state
      setJobs(jobs.map(j => j.id === job.id ? { ...j, views_count: j.views_count + 1 } : j));
    } catch (error) {
    ...
    <div className="jobs-page">
      ...
      <nav className="jobs-nav">
        <div className="jobs-nav-container">
          <div className="jobs-nav-logo">
            <span className="jobs-logo-text">SkillSync</span>
          </div>
          <div className="jobs-nav-links">
            <button onClick={() => navigate('/')} className="jobs-nav-link">
              Home
            </button>
            <button onClick={() => navigate('/dashboard')} className="jobs-nav-link">
              Dashboard
            </button>
            <button className="jobs-nav-link jobs-nav-active">
              Jobs
            </button>
            <button 
              onClick={() => {
              ...
              className="jobs-nav-link jobs-nav-logout"
            >
              Logout
            </button>
          </div>
      ...
      <div className="jobs-hero">
        <div className="jobs-hero-content">
          ...
          <div className="jobs-quick-filters">
            ...
            {(searchTerm || filterJobType !== 'all' || filterExperience !== 'all') && (
              <button 
      ...
      <div className="jobs-content">
        <div className="jobs-container">
          ...
          <div className="jobs-header-section">
            <h2>{filteredJobs.length} Job{filteredJobs.length !== 1 ? 's' : ''} Available</h2>
            {filteredJobs.length !== jobs.length && (
              <p className="filter-info">Filtered from {jobs.length} total jobs</p>
            )}
          </div>
                          ...
                          >
                            ...
                            <div className="job-card-content">
                              ...
                              <div className="job-card-footer">
                                <span className="job-posted">{formatDate(job.created_at)}</span>
                                <span className="job-views-compact">👁️ {job.views_count}</span>
                              </div>
                            </div>
                          </div>
                        ))}
        ...
        <div className="job-modal-overlay" onClick={() => setSelectedJob(null)}>
          <div className="job-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-hero-content">
                {selectedJob.company_logo && (
                  <div className="company-avatar">
                    <img src={selectedJob.company_logo} alt={selectedJob.company_name} />
                  </div>
                )}
                <div className="hero-text">
                  <h1 className="job-title-modal">{selectedJob.title}</h1>
                  <p className="company-name-modal">{selectedJob.company_name}</p>
            ...
            <div className="modal-stats-bar">
              ...
              <div className="stat-pill">
                <div className="stat-info">
                  ...
                  <span className="stat-value">{getJobTypeLabel(selectedJob.job_type)}</span>
                </div>
              </div>
              <div className="stat-pill">
                <div className="stat-info">
                  <span className="stat-label">Experience</span>
                  <span className="stat-value">{getExperienceLabel(selectedJob.experience_level)}</span>
                </div>
              </div>
              {selectedJob.salary_range && (
                <div className="stat-pill highlighted">
                  <div className="stat-info">
                    ...
                    <span className="stat-value">{selectedJob.salary_range}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="modal-body">
              ...
              {/* Skills */}
              {selectedJob.required_skills && JSON.parse(selectedJob.required_skills).length > 0 && (
                <section className="modal-section">
                  <div className="section-header">
                    <span className="section-icon">🔧</span>
                  ...
                  <div className="section-content">
                    <div className="skills-grid">
                      {JSON.parse(selectedJob.required_skills).map((skillId) => {
                        const skill = skills.find(s => s.id === skillId);
                        return skill ? (
                          <span key={skillId} className="skill-chip">{skill.name}</span>
                        ) : null;
                      })}
                    </div>
                  </div>
              ...
              {/* Application Info */}
              {(selectedJob.application_url || selectedJob.application_email) && (
                  ...
                  <div className="section-content">
                    {selectedJob.application_deadline && (
                      <div className="deadline-banner">
                        ...
                        <div>
                          ...
                          <p>{new Date(selectedJob.application_deadline).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedJob.application_url && (
                      <p className="apply-instruction">
                        <strong>Apply Online:</strong> Visit the company's application portal to submit your application.
                      </p>
                    )}
                    
                    {selectedJob.application_email && (
                      <p className="apply-instruction">
                        <strong>Email:</strong> Send your resume to <a href={`mailto:${selectedJob.application_email}`}>{selectedJob.application_email}</a>
                      </p>
                    )}
                  </div>
                </section>
